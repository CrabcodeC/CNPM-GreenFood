...
const navItems = [
  ["vegetable", "Rau Củ", "index.html#vegetable"],
  ["fruit", "Trái Cây", "index.html#fruit"],
  ["fresh", "Thực Phẩm", "index.html#fresh"],
  ["drink", "Đồ Uống", "index.html#drink"],
  ["recipe", "Công thức", "goiymonan.html"],
  ["sale", "Khuyến Mãi", "index.html#sale"],
];

function header(active = "shop") {
  const links = navItems.map(([key, label, href]) => {
    const activeClass = active === key ? "active" : "";
    const saleClass = key === "sale" ? "sale-link" : "";
    return `<a class="${activeClass} ${saleClass}" href="${href}">${label}</a>`;
  }).join("");
    ...
    <header class="site-header">
      <div class="container navbar">
        <a class="brand" href="index.html" aria-label="GreenFood">
          <span class="brand-mark">GF</span><span>GreenFood</span>
        </a>
        ...
        <div class="nav-actions">
          ...
          <button class="account-btn" onclick="location.href='login.html'">Tài khoản</button>
          <button class="cart-btn" aria-label="Giỏ hàng" onclick="location.href='giohang.html'"><span>3</span></button>
        </div>
      </div>
    </header>`;
}
  ...
  return `
    <footer class="site-footer">
      <div class="container footer-grid">
        <div>
          <a class="brand footer-brand" href="index.html"><span class="brand-mark">GF</span><span>Green Food</span></a>
      ...
      <div class="container copyright"> 2024 GreenFood. Bảo lưu mọi quyền. Nguồn gốc địa phương, tầm nhìn toàn cầu.</div>
    </footer>`;
}
  ...
  return `
    <article class="product-card">
      <a class="product-image" href="chi-tiet-san-pham.html?id=${p.id}" style="background-image:url('${p.image}')">
        <button class="wish-btn" type="button" aria-label="Yêu thích"></button>
      ...
      <div class="product-body">
        ...
        <div class="price-row"><div class="price">${money(p.price)}</div><button class="add-btn" onclick="location.href='giohang.html'">Thêm</button></div>
      </div>
    </article>`;
}
...
function sectionBlock(section) {
  ...
  return `
    <section id="${section.id}" class="product-section">
      ...
      <button class="slide-btn slide-prev" aria-label="Sản phẩm trước"></button>
      <div class="product-grid">${products.map(productCard).join("")}</div>
      <button class="slide-btn slide-next" aria-label="Sản phẩm sau"></button>
    </section>`;
}
  ...
  app.innerHTML = `${header("vegetable")}
    <main>
      ...
      <section class="container shop-layout">
        <aside class="panel">
          <h2>Phân loại</h2>
          <div class="category-list">${data.categories.map((c, i) => `<button class="category-btn ${i === 0 ? "active" : ""}" onclick="location.href='index.html#${c.id}'"><span>${c.name}</span><b>${c.count}</b></button>`).join("")}</div>
        ...
        <div class="shop-content">
          <div class="quick-tabs"><button onclick="location.href='index.html'" class="active">Tất cả</button><button onclick="location.href='index.html#vegetable'">Bán chạy</button><button onclick="location.href='index.html#fruit'">Mới về</button><button onclick="location.href='index.html#fresh'">Hữu cơ</button><button onclick="location.href='index.html#sale'">Giảm giá</button><span>Sắp xếp: <b>Mới nhất</b></span></div>
